...
export function Lyric({ height = DEFAULT_LYRIC_HEIGHT, onToggle, animation }: { height?: number; onToggle?: () => void; animation?: any }) {
  ...
  useEffect(() => {
    ...
    console.log(`[歌词] 切歌: ${currentMusic.title} - ${currentMusic.artist} (id=${musicId})`)
    // 内存缓存命中：直接用，不走 loading / 不请求
    const cached = lyricCacheGet(musicId)
    if (cached) { console.log("[歌词] 内存缓存命中"); setResult(cached); return }
    setResult(null)
    setLoading(true)
    ;(async () => {
      try {
        ...
        if (local && (local.synced?.length || local.plain)) {
          console.log(`[歌词] 本地缓存命中: ${local.synced?.length ?? 0} 行`)
          lyricCacheSet(musicId, local)
          if (alive) setResult(local)
          return
        }
        console.log("[歌词] 本地无缓存，请求在线...")
        // 在线兜底
        ...
        console.log(`[歌词] 在线结果: synced=${r.synced?.length ?? 0}, plain=${r.plain ? "有" : "无"}`)
