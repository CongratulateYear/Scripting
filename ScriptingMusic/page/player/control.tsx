...
export function Control() {
  const { isPlaying, isLoading, playMode, queue, currentIndex, currentMusic } = usePlayerState()
  ...
  return (
    ...
    >
      ...
      <HStack tint={"white"} frame={{ maxWidth: "infinity" }}>
        ...
        <Spacer />
        <Button action={() => { if (!isLoading) { isPlaying ? player.pause() : player.play() } }} tint={"white"} disabled={isLoading}>
          {isLoading
            ? <ProgressView controlSize="large" tint="white" frame={{ width: 60, height: 60 }} />
            : <Image
