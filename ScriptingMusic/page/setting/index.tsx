...
function StackView() {
  ...
  const [basePath, setBasePath] = useState(setting.getBasePath())
  ...
  async function loadStorageInfo() {
    await safeRun(async () => {
      ...
      setBasePath(setting.getBasePath())
    ...
    <List>
      ...
      <Section header={<Text>{"存储信息"}</Text>}>
        ...
        <VStack alignment="leading" spacing={4}>
          <Text>{"下载位置"}</Text>
          <Text font="caption" foregroundStyle="secondaryLabel" textSelection={true}>
            {`${basePath}/audios`}
          </Text>
        </VStack>
        <VStack alignment="leading" spacing={4}>
