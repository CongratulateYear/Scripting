...
export type PlayVisualStatus = "idle" | "loading" | "playing"
...
export type RecentlyAddedCardProps = {
  ...
  /** 本地封面是否存在 */
  coverExists: boolean
  playStatus: PlayVisualStatus
  onTap: () => void
}

export function RecentlyAddedCard({ music, coverExists, playStatus, onTap }: RecentlyAddedCardProps) {
      ...
      <VStack alignment="leading" spacing={6} frame={{ width: 130 }}>
        <ZStack alignment="bottomTrailing">
          ...
          {playStatus === "loading" ? (
            ...
            <Image
              systemName={playStatus === "playing" ? "waveform.circle.fill" : "play.circle.fill"}
              font="title2"
              foregroundStyle={playStatus === "playing" ? "systemPink" : "white"}
              padding={6}
            />
          )}
        </ZStack>
        <Text font="subheadline" fontWeight="semibold" lineLimit={1} foregroundStyle={isActive ? "systemPink" : "label"}>
          {music.title}
        </Text>
      </VStack>
    </Button>
...
export function FavoriteSongRow({ music, rank, coverExists, playStatus, showPlayCount, onTap }: FavoriteSongRowProps) {
  const [remoteError, setRemoteError] = useState(false)
  const showLocal = coverExists
  const showRemote = !coverExists && !!music.cover_url && !remoteError
  const isActive = playStatus === "loading" || playStatus === "playing"
  return (
    <HStack spacing={12} padding={{ vertical: 4 }} onTapGesture={onTap}>
      ...
      <VStack alignment="leading" spacing={3}>
        <Text font="body" fontWeight="semibold" lineLimit={1} foregroundStyle={isActive ? "systemPink" : "label"}>
          {music.title}
        </Text>
