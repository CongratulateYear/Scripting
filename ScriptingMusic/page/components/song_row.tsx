...
import { sourceLabel, sourceColor } from "../../class/sources/source_label"
...
export function SongRow(props: SongRowProps) {
  ...
  const playStatus: "idle" | "loading" | "playing" = state.currentMusic?.id === music.id && state.state === "loading"
    ? "loading"
    : state.currentMusic?.id === music.id && state.state === "playing"
      ? "playing"
      : "idle"
  const isPlaying = playStatus === "playing"
  const isLoading = playStatus === "loading"
  ...
  return (
    ...
    >
      ...
      {coverExists?.[music.id]
        ...
        : (fallbackRemoteCover && music.cover_url)
          ...
          : <Image
              ...
              font="title2"
              tint={(isActive ? "accentColor" : placeholderTint) as any}
              frame={{ width: 48, height: 48 }}
            />
      }
      ...
      <VStack alignment="leading" spacing={2}>
        <Text font="headline" lineLimit={1} foregroundStyle={isActive ? "accentColor" : undefined}>
          {music.title}
        </Text>
        {(() => {
          const sub = subtitle ?? music.artist
          const src = sourceLabel(music.provider)
          if (!sub && !trailingMeta && !src) return null
          return (
            <HStack spacing={4} alignment="center">
              ...
              {src ? <Text font="caption2" foregroundStyle={sourceColor(music.provider) as any} fontWeight="medium">{src}</Text> : null}
            </HStack>
          )
        })()}
      </VStack>
      ...
      {/* 播放三态：未播放 / 加载中 / 播放中。独立于收藏和下载状态，不影响收藏逻辑。 */}
      {!isEditing && (
        isLoading ? <ProgressView controlSize="small" /> :
        isPlaying ? <Image systemName="waveform" tint="accentColor" /> :
        <Image systemName="play.circle" foregroundStyle="tertiaryLabel" />
      )}
