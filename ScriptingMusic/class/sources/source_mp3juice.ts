...
const URL_CACHE_TTL = 8 * 60 * 1000
const CDN_CACHE_TTL = 10 * 60 * 1000
...
class SourceMP3Juice implements MusicSource {
  ...
  private cdnCache: { cdn: string; expiresAt: number } | null = null
  ...
  private async getCdn(forceRefresh: boolean = false): Promise<string> {
    const now = Date.now()
    if (!forceRefresh && this.cdnCache && this.cdnCache.expiresAt > now) return this.cdnCache.cdn
    ...
    this.cdnCache = { cdn: j.cdn as string, expiresAt: now + CDN_CACHE_TTL }
  ...
  private cacheKey(info: ResolveInput): string {
    return info.source_id ?? info.id
  }
  ...
  async resolveAudioUrl(info: ResolveInput): Promise<string> {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        const cdn = await this.getCdn(attempt > 0)
        ...
        this.audioUrlCache.set(key, { url: url as string, expiresAt: Date.now() + URL_CACHE_TTL })
        return url as string
      } catch (e) {
        lastError = e
        // CDN 可能失效，清掉缓存后重试一次。
        this.cdnCache = null
        console.log(`[mp3juice] resolve attempt ${attempt + 1} failed: ${e}`)
      }
    }
