...
class Player {
  ...
  // 当前已提交给 AVPlayer 的 source 令牌；onReadyToPlay 若不是最新令牌则忽略，避免旧 source 回调污染状态。
  private activeSourceToken: number = 0
  ...
  async init(): Promise<void> {
    ...
    this.player.onReadyToPlay = () => {
      if (this.activeSourceToken !== this.playToken) {
        console.log(`[Player] 忽略过期 onReadyToPlay: active=${this.activeSourceToken}, current=${this.playToken}`)
        return
      }
    ...
    this.player.onError = (message) => {
      if (this.activeSourceToken !== this.playToken) return
      this.setState("error")
  ...
  private async playMusic(music: Music): Promise<void> {
    ...
    // 立即停止旧音频，避免新歌解析/缓冲期间仍继续播放上一首。
    this.stopProgressTimer()
    try { this.player?.stop() } catch {}
    this.activeSourceToken = 0
    this.setState("loading")
    this.listeners.forEach(l => l.onProgressChange?.(0, music.duration ?? 0))
    this.currentMusic = music
